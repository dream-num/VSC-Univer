...
export class ExcelEditorProvider implements vscode.CustomEditorProvider<ExcelDocument> {
  ...
  private static readonly viewType = 'univer-excel-editor'
  ...
  async openCustomDocument(
    ...
    const document: ExcelDocument = await ExcelDocument.create(uri, openContext.backupId, {
      getFileData: async () => {
        const view = documentViewManager.find(document.uri)
  ...
  public async resolveCustomEditor(
    ...
    const editor = new ExcelDocumentView(this.context, document.uri)
    const excelBuffer = await vscode.workspace.fs.readFile(document.uri)
    editor.panel = webviewPanel
    editor.initialize(excelBuffer)
...
export class ExcelDocument extends Disposable implements vscode.CustomDocument {
  ...
  change(label: string) {
    const view = <ExcelDocumentView>documentViewManager.find(this.uri)
